...
public class RjeMessageParser {
	...
	private void fillChargesValue(ParsedMessage value) {
		List<ParsedField> parsedFields = value.getParsedFields();
		for (ParsedField field : parsedFields) {
			if (field.getFieldCode().contains("71A")) {
				mesg.setMesgCharges(field.getValue());
				break;
			}
		}
