...
@EnableJpaRepositories("com.eastnets.repositories")
public class CustomJpaConfiguration extends JpaBaseConfiguration {
	...
	public LocalContainerEntityManagerFactoryBean localContainerEntityManagerFactory(EntityManagerFactoryBuilder builder, DataSource dataSource) {
		LocalContainerEntityManagerFactoryBean build = builder.dataSource(dataSource).packages("com.eastnets.entities").persistenceUnit("persistenceUnit").properties(initJpaProperties()).build();
		build.setJpaDialect(new EclipseLinkJpaDialect());
		build.setJpaVendorAdapter(new EclipseLinkJpaVendorAdapter());
		return build;
	}

	@Bean("wdTransactionManager")
	...
	public JpaProperties properties() {
		...
		jpaProperties.setDatabasePlatform(watchdogConfiguration.getDatabasePlatform());
