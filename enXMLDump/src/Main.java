...
public class Main {
	...
	public static void main(String[] args) {
		...
		EnLogger.printLn(String.format("\r\nProcessed: %d messages, %d instances, %d interventions, %d appendices", 
				ApplicationMonitor.processedMessages, ApplicationMonitor.processedInstances, 
				ApplicationMonitor.processedInterventions, ApplicationMonitor.processedAppendices));
	...
	private static Connection getConnection( EnXmlDumpConfig globalConfig ) {
		if ( globalConfig.getDBType() == DBType.MSQL  ){
			String url = String.format( "jdbc:sqlserver://%s:%d;DatabaseName=%s;user=%s;Password=%s", 
					globalConfig.getServer(), globalConfig.getPort(),  globalConfig.getDatabaseName(),
					globalConfig.getUser(), globalConfig.getPassword());	
			try {
