...
public class ViewerOracleDAOImp extends ViewerDAOImp {

	private static final long serialVersionUID = -4184310734959286228L;

	@Override
	public void addNewMsgSearchTemplate(MessageSearchTemplate template) throws Exception {

		String insertQuery = " INSERT INTO rMsgSearchTemplates(id,name,template_value,creation_date,created_by ) " + " VALUES (rMsgSearchTemplates_ID.nextval , ?, ?, current_date , ?)";
	...
	public void mapNewMsgSearchTemplateToProfile(MessageSearchTemplate savedTemplate) throws Exception {
		...
		if (template != null) {
			String insertQuery = " INSERT INTO rMsgSearchProfileTemplate(id,template_id,group_id) " + " VALUES (rMsgSearchProfileTemplate_ID.nextval ,?,?)";
	...
	public MessageNote addNewMessageNote(MessageNote messageNote) {
		...
		String insertQuery = "INSERT INTO RMESG_NOTES(NOTE_ID,AID,MESG_S_UMIDL,MESG_S_UMIDH,NOTE,CREATION_DATE,CREATED_BY) VALUES(?,?,?,?,?,?,?)";

		String generatedKeyQuery = "SELECT SEQ_NOTE_ID.NEXTVAL ID FROM DUAL";
	...
	public void insertIntoErrorld(String errorExe, String errorLevel, String errorModule, String errMsg1, String errMsg2) {
		...
		jdbcTemplate.update(insertQuery, new Object[] { mesgID, errorExe, new java.util.Date(), errorLevel, errorModule, errMsg1, "" });
	}
	...
	public List<NotifierMessage> getPendingGpiMesg(String query, int mailAtempt, int confirmAtempt, int timeInterval) throws InterruptedException, SQLException {
		Object[] parameters = new Object[] { mailAtempt, confirmAtempt, timeInterval };
	...
	public MessageKey getMessageKeyByUetr(String uetr, String acountNumber, String msgDirection, boolean isViewerTraker, String fromDate, String toDate) throws SQLException {
		...
		if (isViewerTraker) {
			...
			query = "select aid, mesg_s_umidh, mesg_s_umidl, mesg_crea_date_time from rmesg  "
					+ " where  mesg_crea_date_time between To_DATE( ?, 'YYYY-MM-DD HH24:MI:SS') and To_DATE( ?, 'YYYY-MM-DD HH24:MI:SS') and  MESG_E2E_TRANSACTION_REFERENCE = ? and mesg_type = '103' and  UPPER(mesg_sub_format) like ?  "
	...
	public List<GpiConfirmation> getGpiAgent(String senderBic, String uter, final int timeZoneOffset, List<String> selectedSAA, boolean gSRPRequest, Date mesgCreaDateTime, String... mesgType) {
		String arr[] = mesgType;
		String identifiersQuery = "SELECT * from rMesg where " + getMTConfirmationType(Arrays.asList(arr)) + " and mesg_e2e_transaction_reference= ?  and mesg_frmt_name='Swift'  and  mesg_crea_date_time >= To_DATE( ?, 'YYYY-MM-DD HH24:MI:SS')  "
				+ "and MESG_SLA in('001','002') and MESG_PARSING_DATE_TIME is not null  " + addSenderIfExist(senderBic, gSRPRequest) + " order by mesg_s_umidh desc";
	...
	public List<DetailsHistory> getGpiDetailsHistory(String uter, java.util.Date mesg_crea_date) throws SQLException {
		String query = " select * from rmesg  where MESG_E2E_TRANSACTION_REFERENCE = ? and  mesg_crea_date_time >= To_DATE( ?, 'YYYY-MM-DD HH24:MI:SS')  and mesg_type in ('205','103','199','299','202','192','196') order by mesg_s_umidh ASC";
		...
		List<DetailsHistory> gpiDetelsHistoryList = jdbcTemplate.query(query, new Object[] { uter, mesg_crea_date }, new RowMapper<DetailsHistory>() {
			...
			public DetailsHistory mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				if (rs.getString("MESG_SLA") != null && !rs.getString("MESG_SLA").contains("002")) {
					...
					// In-Case MESG_SND_CHARGES_CURR contains multiple cur
				...
				if (detailsHistory.getSenderDeducts() != null && !detailsHistory.getSenderDeducts().isEmpty()) {
					...
					if (chargesLine.contains(",")) {
						...
						detailsHistory.setSenderDeducts(Utils.formatAmount(new BigDecimal(charges[charges.length - 1].trim()), detailsHistory.getSenderDeductsCur(), getCurrenciesMap()).toString());
					} else {
				...
				if (detailsHistory.getMesgCreaDateTime() != null) {
					String format = sdf.format(detailsHistory.getMesgCreaDateTime());
					detailsHistory.setMesgCreaDateTimeStr(format);
				}
	...
	public List<MessageKey> getMessagesKeys(int aid, boolean isPartitionedDatabase, String fromDate, String toDate, int bulkSize) {
		...
		String query = "select * from ( select m.aid, mesg_s_umidh, mesg_s_umidl,mesg_sender_X1 ,mesg_receiver_swift_address,X_RECEIVER_X1,mesg_crea_date_time, mesg_type, mesg_identifier, mesg_mod_date_time,MESG_E2E_TRANSACTION_REFERENCE ,MESG_SUB_FORMAT, MESG_SLA, m.MESG_NAK_CODE "
				+ " from rMesg m  "
				...
				+ " or (mesg_type='103') or (mesg_type='202') or (mesg_type='205') )" + " and (mesg_parsing_date_time is null or mesg_parsing_date_time < mesg_mod_date_time ) and archived = 0"
				+ " and m.mesg_crea_date_time BETWEEN TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS' ) " + " order by mesg_crea_date_time  ASC ) where  ROWNUM <= ? ";
		List<MessageKey> gpiMessageKeys = jdbcTemplate.query(query, new Object[] { aid, fromDate, toDate, bulkSize }, new RowMapper<MessageKey>() {
