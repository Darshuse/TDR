...
/**
 * AppendixExtDetails POJO
 * @author EastNets
 * @since September 25, 2012
 */
public class AppendixExtDetails implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4643473460492525081L;
	...
	public String getAppeRcvDeliveryStatusFormatted() {
		if (StringUtils.equalsIgnoreCase(appeRcvDeliveryStatus, "RE_N_A")) return "-";
		if (StringUtils.equalsIgnoreCase(appeRcvDeliveryStatus, "RE_UACKED")) return "Receiver Acked";
		return "";
	}
	...
	public static String formatAuthResult(String sts) {
		if ( StringUtils.equalsIgnoreCase(sts, "DLV_N_A"))  return "";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_N_A"))  return "";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_SUCCESS"))  return "Success";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_SUCCESS_OLD_KEY"))  return "Old Key";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_SUCCESS_FUTURE_KEY"))  return "Future KEY";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_FAILURE"))  return "Failure";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_NO_KEY"))  return "No Key";
		if ( StringUtils.equalsIgnoreCase(sts, "AUTH_BYPASSED"))  return "Bypassed";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_NO_RMA_REC"))  return "No RMA Rec";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_RMA_NOT_ENABLED"))  return "RMA Not Enabled";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_RMA_NOT_IN_VALID_PERIOD"))  return "RMA not in valid period";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_INVALID_DIGEST"))  return "Invalid Digest";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_INVALID_SIGN_DN"))  return "Invalid Sign Onx";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_SIG_FAILURE"))  return "Sig Failure";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_INVALID_CERT_ID"))  return "Invalid Cert ID";
		if ( StringUtils.equalsIgnoreCase(sts, "ADK_AUTH_NO_RMAU_BKEY"))  return "No RMAU BKEY";
		return "";
	}
	public static String formatRmaCheckResult(String sts) {
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_N_A"))  return "-";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_SUCCESS"))  return "Success";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_FAILURE"))  return "Failure";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_BYPASSED"))  return "Bypassed";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_NO_REC"))  return "No Record";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_NOT_ENABLED"))  return "Not Enabled";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_NOT_IN_VALID_PERIOD"))  return "Invalid Period";
		if ( StringUtils.equalsIgnoreCase(sts, "RMA_CHECK_NOT_AUTHORISED"))  return "Not allowed";
		return "";
	}
	public static String formatAckSts( String status, String app_name ) {
		if ( !StringUtils.isEmpty( status ))
		{
			if ( StringUtils.equalsIgnoreCase(status, "DLV_N_A"))  return "N/A";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_WAITING_ACK"))  return "Waiting " +  StringUtils.defaultString( app_name )+ " Ack";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_TIMED_OUT"))  return StringUtils.defaultString( app_name ) + " Time out";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_ACKED"))  return StringUtils.defaultString( app_name ) + " Ack";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_NACKED"))  return StringUtils.defaultString( app_name ) + " Nack";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_REJECTED_LOCALLY"))  return "Rejected Locally";
			if ( StringUtils.equalsIgnoreCase(status, "DLV_ABORTED"))  return  StringUtils.defaultString( app_name ) + " Aborted"; 
		}
		//else
		return "-";
	}
