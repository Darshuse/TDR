...
public class MessageReaderSqlDAO extends MessageReaderDAO {

	private static final Logger LOGGER = LogManager.getLogger(MessageReaderSqlDAO.class);

	@Override
	public List<MessageSummaryDTO> getMessageSummaryMesgInformation() {

		LOGGER.trace("MessageReaderSqlDAO - getting message summary list");
	...
	public String generateQuery() {
		...
		LOGGER.trace("Generating MSSQL Reading Query");
		try {

			StringBuilder query = new StringBuilder();
			query.append("SELECT TOP " + globalConfiguration.getAnalyticsParserBulkSize());
			query.append(" M.AID ,M.MESG_S_UMIDL,M.MESG_S_UMIDH,M.MESG_TRN_REF, M.MESG_REL_TRN_REF, M.MESG_CREA_MPFN_NAME,M.MESG_SYNTAX_TABLE_VER,M.mesg_copy_service_id,"
					...
					+ "	M.X_FIN_VALUE_DATE,	M.X_RECEIVER_X1, M.MESG_SENDER_X1, M.MESG_SLA,M.MESG_E2E_TRANSACTION_REFERENCE, M.X_INST0_RP_NAME," + "	M.X_LAST_EMI_APPE_DELV_STS,	M.MESG_STATUS_CODE,	M.MESG_REASON_CODE,	M.MESG_NAK_CODE, M.MESG_CHARGES ,"
					+ " M.FETCH_STATUS ," + getCurrenciesColumnsQuery() + ", " + getAppendixColumnsQuery() + " " + " FROM RMESG M WHERE");
	...
	public String getAppendixColumnsQuery() {
		...
		String[] appendixColumns = { "APPE_SESSION_HOLDER", "APPE_DATE_TIME", "APPE_TYPE" };
	...
	public String getCurrenciesColumnsQuery() {
		...
		for (int i = 0; i < currencyColumns.length; i++) {
			...
			String query = " (SELECT TOP 1" + currencyColumns[i].trim() + " FROM CU WHERE CURRENCYCODE = M.X_FIN_CCY ) AS " + currencyColumns[i].trim() + " ";
