...
public class ReportGenerator {
	...
	public ReportGenerator(SearchDAO searchDAO, XMLReaderDAO xmlReaderDAO, String dateFormat, String timeFormat) {
	...
	public List<String> printReport(List<MessageDetails> messages, boolean isMail, boolean printToFile, boolean expandText, boolean includeHistory, int timeZoneOffset, boolean showMessageText, boolean printExsitPointFormatAsPDF,
		...
		if (!enableCreatedBy) {
			...
			msgsFormatedBuf.append(sdf.format(Calendar.getInstance().getTime()) + " Created by " + " *** " + "\r\n\r\n");
		}
		...
		// List<XMLMessage> xmlMessages = getMxMessages(messages);
		// if (xmlMessages != null && !xmlMessages.isEmpty()) {
		// messages.stream().forEach(n -> n.setXmlMessages(xmlMessages.stream().filter(x -> x.getAid().equals(n.getAid()) && x.getMesgUmidl().equals(n.getMesgUmidl()) &&
		// x.getMesgUmidh().equals(n.getMesgUmidh())).collect(Collectors.toList())));
		// }
		//
		// List<AppendixDetails> appeDetailsList = getAppendixDetails(messages);
		// if (appeDetailsList != null && !appeDetailsList.isEmpty()) {
		// messages.stream()
		// .forEach(n -> n.setAppendixDetailsList(appeDetailsList.stream().filter(x -> x.getAid().equals(n.getAid()) && x.getMesgUmidl().equals(n.getMesgUmidl()) &&
		// x.getMesgUmidh().equals(n.getMesgUmidh())).collect(Collectors.toList())));
		// }
		//
		// List<TextFieldData> textFieldDataList = getTextFieldDataList(messages);
		// if (textFieldDataList != null && !textFieldDataList.isEmpty()) {
		// messages.stream()
		// .forEach(n -> n.setTextFieldDataList(textFieldDataList.stream().filter(x -> x.getAid().equals(n.getAid()) && x.getMesgUmidl().equals(n.getMesgUmidl()) &&
		// x.getMesgUmidh().equals(n.getMesgUmidh())).collect(Collectors.toList())));
		// }

		for (MessageDetails message : messages) {
			String temp = printMessage(message, true);
			resultList.add(temp);
		}
	...
	public String PrintTextInfo(MessageDetails mesg) {
		String result = "";
		String banner;
		String tmpStr = "";
		Integer mesgTypeInt = 0;
		...
		if (mesg.isXML()) {
			...
			if (mesg.getXmlMessages() != null && !mesg.getXmlMessages().isEmpty()) {

				for (XMLMessage xmlMessage : mesg.getXmlMessages()) {
		...
		if (mesgTypeInt == 0 || (mesgTypeInt >= 100 && mesgTypeInt <= 999)) {
			...
			if (expandText == true && (!StringUtils.equalsIgnoreCase(formatName, "internal") && !StringUtils.equalsIgnoreCase(formatName, "MX"))) {// internal messages cannot be expanded
				...
				if (!StringUtils.isEmpty(textBlock) && expandText) {
					try {
						textBlock = searchDAO.getExpandedMesssageText(mesg.getAid(), mesg.getMesgUmidl(), mesg.getMesgUmidh(), mesgVersion, mesg.getMesgType(), textBlock, mesgDate, null, null);
					} catch (Exception e) {
					}
			...
			if (StringUtils.isEmpty(textBlock)) {
				for (int i = 0; i < mesg.getTextFieldDataList().size(); i++) {
					textBlock += ":" + mesg.getTextFieldDataList().get(i).getFieldCode().toString() + ":" + StringUtils.defaultString(mesg.getTextFieldDataList().get(i).getValue()) + "\n";
	...
	private String printInputMessageTrailer(MessageDetails mesg) {
		...
		if (mesg.getAppendixDetailsList() != null && !mesg.getAppendixDetailsList().isEmpty()) {
			for (AppendixDetails appeDetails : mesg.getAppendixDetailsList()) {
				if ("SWIFT".equalsIgnoreCase(appeDetails.getAppeIAppName()) && "APPE_EMISSION".equalsIgnoreCase(appeDetails.getAppeType())) {
					try {
						AppendixExtDetails appeExtDetails = searchDAO.getAppendixDetails(mesg.getAid(), mesg.getMesgUmidl(), mesg.getMesgUmidh(), mesg.getMesgCreaDateTimeOnDB(), 0, appeDetails.getAppeSeqNbr(), appeDetails.getAppeDateTimeOnDB(),
								timeZoneOffset);
	...
	private String printHeaderInfo(MessageDetails mesg) {
		...
		if (mesg.isXML()) {
			...
			textBlock = StringUtils.defaultString(SearchUtils.convertClob2String(mesg.getTextDataBlock()), "");
		...
		if (StringUtils.isNumeric(mesg_type)) {
			...
			if (msgTypeInt >= 100 && msgTypeInt <= 999) {
				Pair<String, String> pair = searchDAO.getMTExpantion(mesg.getAid(), mesg.getMesgUmidl(), mesg.getMesgUmidh(), mesg.getMesgCreaDateTimeOnDB(), mesg_type);
	...
	private String expandBIC(String bicCode, String tabLength) {
		...
		corrInfo = searchDAO.getCorrInfo(corrInfo);
	...
	private String printTransmissionInfo(MessageDetails mesg) {
		...
		InstanceTransmissionPrintInfo instanceTransmissionPrintInfo = searchDAO.getInstanceTransmissionPrintInfo(mesg.getAid(), mesg.getMesgUmidl(), mesg.getMesgUmidh(), mesg.getMesgCreaDateTimeOnDB(), 0);
		...
		if (instanceTransmissionPrintInfo.getAppeIappName() != null) {
			if (bInputMsg) {
				mesgInputReference = SearchUtils.formatMessageInputReference(instanceTransmissionPrintInfo.getAppeDateTime(), instanceTransmissionPrintInfo.getAppeSessionHolder(), instanceTransmissionPrintInfo.getAppeSessionNbr(),
				...
				mesgOutputReference = SearchUtils.formatMessageOutputReference(instanceTransmissionPrintInfo.getAppeLocalOutputTime(), instanceTransmissionPrintInfo.getAppeSessionHolder(), instanceTransmissionPrintInfo.getAppeSessionNbr(),
			...
			if (!StringUtils.isEmpty(SearchUtils.formatAckNak(instanceTransmissionPrintInfo.getAppeNetworkDeliveryStatus()))) {
				mesgTransmissionInfoText = mesgTransmissionInfoText + " (" + SearchUtils.formatAckNak(instanceTransmissionPrintInfo.getAppeNetworkDeliveryStatus()) + ")";
			}
			if (nothingPrinted) {
				result += printBanner(banner);
			...
			if (bInputMsg) {
				if (StringUtils.equalsIgnoreCase(notification, "Transmission")) {
					...
					mesgTransmissionInfoText = SearchUtils.formatAckSts(instanceTransmissionPrintInfo.getAppeNetworkDeliveryStatus(), "Network");
					result += printText(mesgTransmissionInfoText, true);
				} else {
					if (StringUtils.equalsIgnoreCase(notification, "Delivery")) {
						...
						result += printText(mesgTransmissionInfoText, false);
						result += positionText(" : ", eyeCatcher + "Correspondent Input Reference", mesgTransmissionInfoText);
						mesgTransmissionInfoText = SearchUtils.formatAckSts(instanceTransmissionPrintInfo.getAppeRcvDeliveryStatus(), "Network");
						result += printText(mesgTransmissionInfoText, true);
					}
				}
			}
	...
	List<String> breakLongLine(String longLine, int maxLineLength) {
		...
		while (longLine.length() > maxLineLength) {
			...
			while (spaceIndex >= 0 && spaceIndex < maxLineLength) {// keep going through spaces till you find the last
																	// space before the
			...
			if (lastSpaceIndex < maxLineLength && lastSpaceIndex > 0) { // if last space was found in the valid range
																		// specified for the line size, then use it to
																		// cut the string
	...
	// * @return
	// */
	// private List<AppendixDetails> getAppendixDetails(List<MessageDetails> messages) {
	// List<AppendixDetails> appeDetailsList = null;
	// if (messages != null && !messages.isEmpty()) {
	// String buildCompsiteKeyForAppendixDetails = exportingUtils.buildCompsiteKeyForMessageDetails(messages);
	// appeDetailsList = searchDAO.getAppendixList(buildCompsiteKeyForAppendixDetails);
	// }
	//
	// return appeDetailsList;
	// }
	//
	// /**
	// * @param messages
	// *
	// */
	// private List<XMLMessage> getMxMessages(List<MessageDetails> messages) {
	// List<XMLMessage> xmlMessages = null;
	//
	// List<MessageDetails> mxMessages = messages.stream().filter(x -> x.isXML()).collect(Collectors.toList());
	// if (mxMessages != null && !mxMessages.isEmpty()) {
	// String buildCompsiteKeyForMxMessages = exportingUtils.buildCompsiteKeyForMessageDetails(mxMessages);
	// xmlMessages = xmlReaderDAO.getXMLMessage(buildCompsiteKeyForMxMessages, true);
	// }
	//
	// return xmlMessages;
	// }
	...
	//
	// List<MessageDetails> swiftMessages = messages.stream().filter(x -> !x.isXML() && x.getMesgType() != null && !SearchUtils.IsSystemMessage(x.getMesgType()) &&
	// NumberUtils.isNumber(x.getMesgType())
